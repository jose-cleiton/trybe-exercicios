...
import {  frameMocForChangeWithId, frameMock, frameMockForChancge, frameMockWithId } from '../../mocks/frameMock';
...
describe('Frame Model', () => {
  const frameModel = new FrameModel();
	...
	before(() => {
		...
		sinon.stub(Model, 'findByIdAndUpdate').resolves(frameMocForChangeWithId);
	});
...
describe('changing a frame', () => {
	...
	it('successfully changed', async () => {
		const frameChanged = await frameModel.update('62cf1fc6498565d94eba52cd', frameMockForChancge);
		expect(frameChanged).to.be.deep.equal(frameMocForChangeWithId);
	})
